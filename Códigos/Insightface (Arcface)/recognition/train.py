...
def parse_args():
    ...
    parser.add_argument('--per-batch-size', type=int, default=default.per_batch_size,
...
def get_symbol(args):
    ...
    if config.loss_name == 'softmax':  # softmax
        ...
        if config.fc7_no_bias:
            fc7 = mx.sym.FullyConnected(data=embedding, weight=_weight, no_bias=True,
        ...
        positive = mx.symbol.slice_axis(nembedding, axis=0, begin=args.per_batch_size //
                                        3, end=2 * args.per_batch_size // 3)
        negative = mx.symbol.slice_axis(nembedding, axis=0, begin=2 *
                                        args.per_batch_size // 3, end=args.per_batch_size)
...
def train_net(args):
    ...
    logging.info('Called with argument: ' + str(args) + ' ' + str(config))
    ...
    if len(args.pretrained) == 0:
        ...
        logging.info('loading {} {}'.format(args.pretrained, args.pretrained_epoch))
    ...
    if config.count_flops:
        ...
        logging.info('Network FLOPs: {}'.format(_str))
    ...
    def _batch_callback(param):
        ...
        if mbatch >= 0 and mbatch % args.verbose == 0:
            ...
            logging.info('[{}]Accuracy-Highest: {:.5f}'.format(mbatch, highest_acc[-1]))
